 ...
 * references remain.
 */
void remove_state_impl_ref(struct state_impl *state_impl) {
  state_impl->num_refs--;
  if (state_impl->num_refs == 0) {
    ...
    if (has_ref) {
      ...
      for (int i = state_impl->frames_size - 1; i > last_ref_index; i--) {
        undo_move(state_impl->frames[i].move, state_impl->occupied);
      }
      state_impl->frames_size = last_ref_index + 1;
    } else {
...
state_t make_move(move_t move, state_t state) {
  ...
  new_state_impl->frames[state.index] = (struct history_frame){move, new_state_impl->num_refs - 1};
  new_state_impl->num_refs = 1;
...
search move_t valid_move(state_t state) {
  choose signed from = range(0, state.impl->size);
...
void print_state(state_t state) {
  ...
  remove_state_impl_ref(new_state_impl);
}
